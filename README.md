...
PowerLit 的产品思路吸收了多个成熟项目和生态的优秀实践，并围绕电力系统文献工作流做了重新组合：

| 项目或生态 | 借鉴点 | PowerLit 的落地 |
| --- | --- | --- |
| `Zotero` | 文献条目、附件和引用信息的统一管理 | 以 DOI 和题录为中心，把 PDF、解析文本、分析结果和下载状态关联起来 |
| `Obsidian` | Markdown 知识库和可长期维护的个人研究笔记 | 生成适合阅读、链接和二次整理的 Markdown / Obsidian 笔记 |
| `Crossref`、`OpenAlex`、`Unpaywall` | 开放学术元数据和开放获取线索 | 多源检索、题录归档、全文候选解析和下载队列生成 |
| `Semantic Scholar` / `OpenScholar` 类研究助理思路 | 面向论文语义理解、主题聚合和 RAG 的研究辅助体验 | 以主题包、工作区、论文卡片和后续 RAG 索引承接分析结果 |
| `project-template-lite` | GitHub 首页清晰表达项目定位、材料职责和使用流程 | README 采用说明页结构，让新读者快速判断项目用途、入口和边界 |
| `FastAPI` + `Typer` | Web API 和命令行工具并行提供产品入口 | 同一套服务能力同时暴露为 Web UI、HTTP API 和 CLI |

这些灵感共同指向一个目标：把“找文献、管 PDF、读全文、做笔记、写分析”整理成一套可以重复运行的研究基础设施。
