...
PowerLit 的灵感主要来自 GitHub 上大量同类型的 paper database、literature manager、literature review pipeline 和 AI research workbench 项目。它们展示了一个很清晰的趋势：研究者希望把论文条目、PDF、笔记、检索记录和分析结果变成可检索、可维护、可交接的项目资产。

| GitHub 同类项目类型 | 代表项目 | 给 PowerLit 的启发 | PowerLit 的产品化落点 |
| --- | --- | --- | --- |
| 开源论文管理器 | [Future-Scholars/paperlib](https://github.com/Future-Scholars/paperlib) | 用独立工具管理 academic papers、元数据和附件 | 保留本地论文库核心能力，并进一步接入电力系统主题、下载队列和 AI 分析链路 |
