...
[项目初心](#项目初心) · [使用场景](#使用场景) · [同类项目参考](#同类项目参考) · [项目特色](#项目特色) · [五分钟上手](#五分钟上手) · [PowerLit 管理哪些材料](#powerlit-管理哪些材料) · [常见任务怎么做](#常见任务怎么做) · [推荐工作流](#推荐工作流) · [目录结构](#目录结构) · [配置说明](#配置说明) · [Web 与 API](#web-与-api) · [提交边界](#提交边界) · [版本说明](#版本说明)

---

## 项目初心

PowerLit 最初是为了解决一个很具体的问题：降低 AI 辅助科研时的幻觉风险，让 AI 在检索、综述、写作和分析时引用真实存在、可以追溯到 DOI、题名、期刊和 PDF 的文献。

随着实际使用推进，这个本地文献库逐渐展现出更丰富的价值：它可以支撑文献综述、辅助论文写作、沉淀研究组长期阅读材料，并帮助研究者更快地分析、理解和比较文献。PowerLit 中的论文写作 skill 也来自这条路线：先建立可信文献底座，再让 AI 在明确来源和结构化材料之上参与写作。

因此，PowerLit 的目标是一套可验证、可积累、可扩展的科研文献工作流。它把真实文献、元数据、PDF、解析文本、笔记、分析结果和后续写作材料连接起来，让 AI 成为有来源约束的研究助理。
...
PowerLit 的 README 在表达产品定位时，参考了 GitHub 上若干同类 paper database、literature manager、literature review pipeline、RAG research assistant 和 PDF parsing 项目。这些项目提供了横向参照：论文集合可以有稳定条目、可追溯元数据、全文解析结果、代码或复现材料，以及面向 AI 检索和分析的结构化出口。

| 参考方向 | 代表项目 | 可借鉴做法 | PowerLit 的产品化落点 |
| --- | --- | --- | --- |
...
| PDF 转 Markdown / JSON 工具 | [opendatalab/MinerU](https://github.com/opendatalab/MinerU) | 将复杂 PDF 等文档解析为适合 LLM 工作流使用的 Markdown / JSON | 通过 MinerU 官方 API 把解析产物关联到 DOI、期刊目录、Obsidian 笔记和分析记录 |

PowerLit 在这个参照系中聚焦电力系统方向，把元数据可信性、本地/NAS 文件管理、人工下载现实、中文期刊入口、AI 笔记和批量分析整合进同一套轻量工具。
...
- **毫秒级证据检索测试版**：`powerlit rag evidence` 使用 SQLite FTS5 在已解析 JSON 中快速检索可追溯证据包，供 PowerLit 写作 skill 使用。
...
POWERLIT_JSON_ROOT=<NAS_OR_STORAGE_ROOT>/literature/json
POWERLIT_INDEX_ROOT=<NAS_OR_STORAGE_ROOT>/literature/index/evidence
```
...
`0.2.0b1` 新增本地证据检索测试版。它只做 SQLite FTS5 检索，不调用 LLM，也不计算 embedding，适合给 `powerlit-power-systems-writing-skills` 提供 citation、novelty 和 evidence pack。

```powershell
.\.venv\Scripts\powerlit rag build-evidence-index --force
.\.venv\Scripts\powerlit rag evidence "Z-Bus voltage unbalance" --top 20 --json
.\.venv\Scripts\powerlit rag evidence-status
```

如果只想索引目标期刊目录：

```powershell
.\.venv\Scripts\powerlit rag build-evidence-index --venue-folder ieee_tsg --venue-folder ieee_tpwrs --force
.\.venv\Scripts\powerlit rag evidence "chance-constrained OPF" --venue-folder ieee_tsg --json
```
...
| `POWERLIT_JSON_ROOT` | 快速证据检索读取的 PowerLit JSON 根目录 |
| `POWERLIT_INDEX_ROOT` | 快速证据检索 SQLite FTS 索引目录 |
